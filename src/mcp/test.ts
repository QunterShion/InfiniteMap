...
import { getKmSummary, listTodos, getNodeById, getNodePath } from './services/kmFileReader';
...
try {
  const summary = await getKmSummary(testFile);
...
console.log('\n--- 测试 km_list_todos ---');
try {
  const todos = await listTodos(testFile);
...
try {
  const todos = await listTodos(testFile);
  if (todos.length > 0) {
    const node = await getNodeById(testFile, todos[0].nodeId);
    if (node) {
      console.log(`节点: ${node.data.text}`);
      console.log(`标签: ${JSON.stringify(node.data.resource)}`);
      const nodePath = await getNodePath(testFile, todos[0].nodeId);
...
console.log('\n--- 测试 km_validate ---');
try {
  const validation = await validateKmFile(testFile);
...
try {
  ...
  if (todos.length > 0) {
    ...
    const result = await markNodesDone(testFile, nodeIds, true);
