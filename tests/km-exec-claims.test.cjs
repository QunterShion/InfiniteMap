...
test('claim only targets leaf todos and skips parents', (t) => {
  ...
  const result = claimTodos(filePath, 'agent-a');
...
test('second worker cannot claim nodes under an active lease', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());

  const first = claimTodos(filePath, 'agent-a', { nodeIds: ['leaf-a'] });
  assert.equal(first.claimedCount, 1);

  // 自动分派跳过已被认领的 leaf-a
  const second = claimTodos(filePath, 'agent-b');
...
test('complete claim marks nodes done atomically and updates sidecar', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const claim = claimTodos(filePath, 'agent-a');

  const dryRun = completeClaim(filePath, claim.claimId, undefined, true);
  assert.equal(dryRun.dryRun, true);
  assert.equal(dryRun.completedCount, 2);
  assert.deepEqual(listTodos(filePath).map((todo) => todo.nodeId), ['parent', 'leaf-a', 'leaf-b']);

  const result = completeClaim(filePath, claim.claimId);
...
test('complete claim rejects when node was modified after claiming', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const claim = claimTodos(filePath, 'agent-a', { nodeIds: ['leaf-a'] });
...
test('expired lease returns task to pending and blocks completion', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const claim = claimTodos(filePath, 'agent-a', { nodeIds: ['leaf-a'] });
  ...
  assert.throws(() => completeClaim(filePath, claim.claimId), /租约已过期/);
  assert.throws(
  ...
  // 过期后其他执行者可以重新认领
  const reclaimed = claimTodos(filePath, 'agent-b', { nodeIds: ['leaf-a'] });
  assert.equal(reclaimed.claimedCount, 1);
  assert.equal(readExecState(filePath).tasks['leaf-a'].workerId, 'agent-b');
});

test('renew extends lease for the original worker only', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const claim = claimTodos(filePath, 'agent-a');
...
test('release and fail return tasks to pending for re-claiming', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const claim = claimTodos(filePath, 'agent-a');

  const released = releaseClaim(filePath, claim.claimId, { nodeIds: ['leaf-a'] });
  assert.equal(released.state, 'released');

  const failed = releaseClaim(filePath, claim.claimId, {
    nodeIds: ['leaf-b'],
    failReason: '输出物验证不通过',
  });
  ...
  // 释放和失败的任务都可以被重新认领
  const reclaimed = claimTodos(filePath, 'agent-b');
  assert.deepEqual(reclaimed.tasks.map((task) => task.nodeId), ['leaf-a', 'leaf-b']);
});

test('legacy mark done is blocked for actively claimed nodes', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const claim = claimTodos(filePath, 'agent-a', { nodeIds: ['leaf-a'] });
  ...
  releaseClaim(filePath, claim.claimId);
  const result = markNodesDone(filePath, ['leaf-a'], false);
...
test('stale lock file is preempted and lock is cleaned after critical section', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  ...
  const value = withKmFileLock(filePath, () => {
  ...
  assert.throws(() => claimTodos(filePath, 'agent-a'), /获取 KM 文件锁超时/);
});

test('claim with stale expectedKmRevision is rejected', (t) => {
  const filePath = createFixture(t, parentWithTwoLeaves());
  const staleRevision = getKmFileRevision(filePath);

  markNodesDone(filePath, ['leaf-b'], false);
